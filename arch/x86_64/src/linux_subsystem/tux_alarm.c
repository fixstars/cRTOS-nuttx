/****************************************************************************
 *  arch/x86_64/src/linux_subsystem/tux_alarm.c
 *
...
static inline
void translate_from_tux_sigaction(struct sigaction *out,
                                  const struct tux_sigaction *in)
{
...
static inline
void translate_to_tux_sigaction(struct tux_sigaction *out,
  ...
  if (second == 0)
  ...
  else
    {
      ...
      if (!ret)
          ...
          svcerr("Timer Create Failed %d\n", ret);

      if (!ret)
          ret = tip.it_value.tv_sec;
      else
          svcerr("Timer set Failed\n");
    }
...
long tux_rt_sigaction(unsigned long nbr, int sig,
                      const struct tux_sigaction *act,
                      struct tux_sigaction *old_act,
                      uint64_t set_size)
{
  int ret;
...
long tux_rt_sigprocmask(unsigned long nbr, int how,
...
long tux_rt_sigtimedwait(unsigned long nbr,
                         const sigset_t *uthese, tux_siginfo_t *uinfo,
