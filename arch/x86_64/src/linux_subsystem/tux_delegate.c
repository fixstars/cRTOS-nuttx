/****************************************************************************
 *  arch/x86_64/src/linux_subsystem/tux_delegate.c
 *
                          ...
                          uintptr_t parm6)
{
  ...
  ret = ((syscall_t) \
         ...
         (linux_syscall_number_table[nbr] - CONFIG_SYS_RESERVED,
                          ...
                          uintptr_t parm6)
{
  ...
  if (rtcb->xcp.is_linux && rtcb->xcp.linux_sock)
    ...
    {
      _err(
          "Non-linux process calling linux syscall"
          " or invalid sock fd %d, %d\n",
                          ...
                          uintptr_t parm6)
{
  ...
  else if (parm1 < CONFIG_TUX_FD_RESERVE)
    {
      /* Lower parts should be delegated */
      ret = tux_delegate(nbr, parm1, parm2, parm3, parm4, parm5, parm6);
    }
  else
    {
  ...
  if(nbr == 2)
    {
      ...
      new_flags = 0;
      if (parm2 & TUX_O_ACCMODE)
          new_flags |= O_ACCMODE;

      if (parm2 & TUX_O_WRONLY)
          new_flags |= O_WRONLY;
      else if (parm2 & TUX_O_RDWR)
          new_flags |= O_RDWR;
      else
          new_flags |= O_RDONLY;

      if (parm2 & TUX_O_CREAT)
          new_flags |= O_CREAT;
      if (parm2 & TUX_O_EXCL)
          new_flags |= O_EXCL;
      if (parm2 & TUX_O_NOCTTY)
          new_flags |= O_NOCTTY;
      if (parm2 & TUX_O_TRUNC)
          new_flags |= O_TRUNC;
      if (parm2 & TUX_O_APPEND)
          new_flags |= O_APPEND;
      if (parm2 & TUX_O_NONBLOCK)
          new_flags |= O_NONBLOCK;
      if (parm2 & TUX_O_DSYNC)
          new_flags |= O_DSYNC;
      if ((parm2 & TUX_O_SYNC) == TUX_O_SYNC)
          new_flags |= O_SYNC;
      if (parm2 & TUX_O_DIRECT)
          new_flags |= O_DIRECT;
      if ((parm2 & TUX_O_TMPFILE) == TUX_O_TMPFILE)
          return -EINVAL;
      if (parm2 & TUX_O_NDELAY)
          new_flags |= O_NDELAY;
                          ...
                          uintptr_t parm6)
{
  ...
  if (parm1 < CONFIG_TUX_FD_RESERVE && parm2 < CONFIG_TUX_FD_RESERVE)
  ...
  else if (parm1 >= CONFIG_TUX_FD_RESERVE && parm2 >= CONFIG_TUX_FD_RESERVE)
    {
      ret = -1;
      ret = tux_local(nbr,
                      parm1 - CONFIG_TUX_FD_RESERVE,
                      parm2 - CONFIG_TUX_FD_RESERVE,
                      parm3, parm4, parm5, parm6) + CONFIG_TUX_FD_RESERVE;
    }
  else if (parm1 < CONFIG_TUX_FD_RESERVE && parm2 >= CONFIG_TUX_FD_RESERVE)
    {
      ret = -EINVAL;
