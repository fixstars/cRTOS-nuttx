/****************************************************************************
 *  arch/x86_64/src/linux_subsystem/tux_futex.c
 *
...
long tux_futex(unsigned long nbr,
               int32_t *uaddr, int opcode,
               uint32_t val, uintptr_t val2, int32_t *uaddr2, uint32_t val3)
{
  ...
  switch(opcode)
    {
      case FUTEX_WAIT:
          ...
          while ((futex_hash_table[hv].key != 0) &&
          ...
          if (*uaddr == val)
            {
              if (futex_hash_table[hv].key == 0)
                  sem_init(&(futex_hash_table[hv].sem), 0, 0);
      ...
      case FUTEX_WAKE_BITSET | FUTEX_CLOCK_REALTIME:
          ...
          for (int i = 0; i < FUTEX_HT_SIZE; i++)
            {
              if ((futex_hash_table[hv].key == ((uint64_t)paddr)) &&
                  (futex_hash_table[hv].bitmask & val3))
                {
                  /* wake this */
      ...
      case FUTEX_CMP_REQUEUE:
          ...
          if (*uaddr != val3)
          ...
          else
            {
              ...
              while ((futex_hash_table[hv2].key != 0) &&
