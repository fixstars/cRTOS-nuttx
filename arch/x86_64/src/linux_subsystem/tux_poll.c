/****************************************************************************
 *  arch/x86_64/src/linux_subsystem/tux_poll.c
 *
...
int pollfd_translate2local(struct pollfd* out,
                           struct tux_pollfd* in, tux_nfds_t nfds)
{
  int i;
...
int pollfd_translate2tux(struct tux_pollfd* out,
                         struct pollfd* in, tux_nfds_t nfds)
{
  int i;
...
long tux_poll(unsigned long nbr,
              struct tux_pollfd *fds,
              tux_nfds_t nfds, int timeout)
{
  int ret;
  int i, j, k;
  ...
  for (i = 0; i < nfds; i++)
    {
        ...
        if(((fds[i].fd > 2) && (fds[i].fd < CONFIG_TUX_FD_RESERVE)) ||
           ((fds[i].fd >= 0) && (fds[i].fd <= 2) &&
            (rtcb->xcp.fd[fds[i].fd] == fds[i].fd)
           ))
          {
            tux_count++;
          }
  ...
  if (tux_count)
    {
      tux_fds =
        (struct tux_pollfd*)kmm_malloc(sizeof(struct tux_pollfd) * (tux_count));
      if(!tux_fds)
        {
  ...
  if (local_count)
    {
      tux_local_fds =
        (struct tux_pollfd*)kmm_malloc(sizeof(struct tux_pollfd) * (local_count));
      if (!tux_local_fds)
        {
      ...
      if (mixing && !(local_fds[local_count - 1].revents & POLLIN))
        {
          ...
          /* The shadow process was yet blocking,
           * we need to do something about it
           * However, it might be unblocked now. Try to unblock it
           */
           ...
           * This looks idiotic, try to make the shadow not transmitting
           * the additional packet XXX
           */
